<script lang="ts">
	...
	const widthScale = $derived(
		scaleLinear().domain([0, maxItems]).range([4, barMaxWidth]).clamp(true)
	);
</script>
		...
		{#each data.rows as row (row.label)}
			<div class="row">
				<div class="row-header">
					...
					<span class="row-count">{row.items.length} {row.items.length === 1 ? 'item' : 'items'}</span>
				</div>
				<svg width={barMaxWidth} height={barHeight} role="img" aria-label="{row.label} item count">
					<rect class="row-bar" width={widthScale(row.items.length)} height={barHeight} rx={3} />
				</svg>
				<ul>
